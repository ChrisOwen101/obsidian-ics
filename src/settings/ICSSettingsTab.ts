...
import { Calendar } from "./ICSSettings";
...
export default class ICSSettingsTab extends PluginSettingTab {
  ...
  async display(): Promise<void> {
    ...
    const calendarContainer = containerEl.createDiv("ics-setting-calendar");
    ...
    for (let a in this.plugin.data.calendars) {
      ...
      let calEl = await getCalendarElement(calendar.icsName, calendar.icsUrl);
...
class SettingsModal extends Modal {
  ...
  async display() {
    ...
    let calendarPreview = await getCalendarElement(this.icsName, this.icsUrl);
