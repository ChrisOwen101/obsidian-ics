...
function findRecurringEvents(icsArray: any[], dayToMatch: string) {
  ...
  for (const k in icsArray) {
    ...
    const duration =
      Number.parseInt(endDate.format("x"), 10) -
    ...
    if (typeof event.rrule !== "undefined") {
      ...
      for (const i in dates) {
        ...
        let endDate = moment(
          Number.parseInt(startDate.format("x"), 10) + curDuration,
          "x"
        );
        ...
        if (includeRecurrence === true) {
          ...
          matchingRecurringEvents.push(
            cloneRecurringEvent(curEvent, startDate, endDate)
          );
        }
      }
    }
